...
export const Login = async (req, res) => {
    ...
    try {
        ...
        const isMatch = await bcryptjs.compare(password, user.password);   // to match ki entered password wai hai na jo DB me stored hai
        if (!isMatch) {
            return res.status(401).json({
                message: "Incorrect email or password",
                success: false
...
export const getMyProfile = async (req, res) => {
    try {
        ...
        const user = await User.findById(id).select("-password"); // to remove password in response
